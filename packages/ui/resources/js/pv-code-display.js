(() => {
  ...
  // resources/js/src/pv-code-display.entry.js
  ...
  function highlightCodeDisplays(root = document) {
    root.querySelectorAll("[data-pv-code-display] code").forEach((block) => {
      const language = [...block.classList].map((className) => className.match(/^language-(.+)$/)?.[1]).find(Boolean);
      const grammar = language ? import_prismjs.default.languages[language] : null;
      if (!grammar) {
        return;
      }
      const source = block.textContent ?? "";
      block.innerHTML = import_prismjs.default.highlight(source, grammar, language);
    });
  }
