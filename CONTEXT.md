...
- Models: `$name` registers a table; `$inherit` on a class that **extends** the target model merges fields, replaces the registry entry (so `parent::` / stacked extensions work), and runs `ALTER TABLE` on install (`partners_ext` fixture)
