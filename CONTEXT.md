...
- Views: module `DATA` files (`views/*.php`) return `ViewsData::make()->views(…)->inherits(…)` (like `Manifest`); plain arrays still work; synced to `ir.ui.view` on install/sync; `ViewRegistry` resolves inheritance via `ArchResolver` + `ArchOperations`
