...
- Views: module `DATA` files return `ViewsData::make()->views(…)->inherits(…)->menus(…)`; synced to `ir.ui.view` / `ir.ui.menu`; shell nav from `MenuTreeBuilder` + `MenuLayoutContext` (Blade layout `velm-filament::layouts.velm-app`, primary `#286291`, accent `#F1511B`); Filament renders arch pages inside the shell; `GET /api/views/{module}/{name}` returns resolved arch JSON; `GET /api/records?model=&domain=&fields=` lists rows; `POST/PATCH/DELETE /api/records` for CRUD; `GET /api/m2o/search?model=&q=` for combobox options (`velm-web`)
