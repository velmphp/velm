...
- Views: module `DATA` files return `ViewsData::make()->views(…)->inherits(…)->menus(…)`; synced to `ir.ui.view` / `ir.ui.menu`; shell nav from `MenuTreeBuilder` + `MenuLayoutContext` (Blade layout `velm-filament::layouts.velm-app`, PyVelm semantic tokens + indigo primary); Filament renders arch pages inside the shell; `GET /api/views/{module}/{name}` returns resolved arch JSON; `GET /api/records?model=&domain=&fields=` lists rows; `POST/PATCH/DELETE /api/records` for CRUD (`velm-web`)
