...
Velm **1.0.0** is stable on Packagist. From any directory:
...
Replace `my_app` with your project directory name. Composer installs Laravel, `velmphp/framework`, and transitive `velmphp/*` libraries at **`^1.0`**.

To pin an exact version (recommended for production deploys):
...
Packagist reads **git tags** on the mirror repos (`velmphp/app`, `velmphp/framework`, …). Older **1.0.0-rc*** tags remain available if you need them (`composer create-project velmphp/app my_app v1.0.0-rc3 -s rc`).
...
See the full [Production](./production) guide (attachments disk, DB choice, cron, smoke checks).
