(() => {
  ...
  var unsetAllMarks = (options = {}) => ({ tr: tr2, dispatch: dispatch2, editor }) => {
...
${indentedChild}`;
  ...
  var hoistBranchingNestedList = (state, dispatch2, itemName, wrapperNames) => {
  ...
  var index_default3 = Superscript;
...
3. "-" cannot repeat`);
  ...
  var AttributeParser = class {
    ...
    static extractWidthFromStyle(style2) {
      const width = style2.match(/width:\s*([0-9.]+)px/);
      return width ? width[1] : null;
    }
  };
  function clampWidth(width, limits) {
    const { minWidth, maxWidth } = limits;
    const absoluteMin = minWidth !== void 0 ? Math.max(0, minWidth) : 0;
    let clampedWidth = Math.max(absoluteMin, width);
    if (maxWidth !== void 0 && clampedWidth > maxWidth) {
  ...
  var ImageNodeView = class {
    constructor(context, inline, resizeLimits = {}) {
      ...
      this.handleDocumentClick = (e) => {
        ...
        const isClickInside = !!target && (this.elements.container.contains(target) || !!target.closest("[data-resize-image-ui]"));
        if (!isClickInside) {
      ...
      this.update = (node) => {
        ...
        const imgAttrChanged = prev.attrs.src !== node.attrs.src || prev.attrs.alt !== node.attrs.alt || prev.attrs.title !== node.attrs.title;
  ...
  var imageResizeConfig = {
    ...
    addOptions() {
      var _a;
      return Object.assign(Object.assign({}, (_a = this.parent) === null || _a === void 0 ? void 0 : _a.call(this)), { inline: false, minWidth: void 0, maxWidth: void 0 });
    },
    addAttributes() {
      var _a;
  ...
  var ImageResize = index_default6.extend(imageResizeConfig);
    ...
    constructor() {
      ...
      this.dispatchNodeView = () => {
        ...
        if (typeof getPos === "function") {
          ...
          const tr2 = view.state.tr.setNodeMarkup(getPos(), null, newAttrs);
          const { selection } = view.state;
          const newSelection = selection instanceof NodeSelection ? NodeSelection.create(tr2.doc, selection.from) : selection.map(tr2.doc, tr2.mapping);
          view.dispatch(tr2.setSelection(newSelection));
        }
      };
    ...
    addCommands() {
      return {
        ...
        addCaption: (caption = "") => ({ chain, state }) => {
          ...
          return chain().insertContentAt({ from: from2, to: from2 + node.nodeSize }, figureNode.toJSON(), {
            updateSelection: false
          }).command(({ commands }) => {
            return commands.setTextSelection(from2 + 2);
          }).focus().run();
        },
        /**
         ...
         */
        removeCaption: () => ({ state, dispatch: dispatch2 }) => {
          const found2 = findEnclosingFigure(state);
          if (!found2)
            return false;
          if (dispatch2) {
            const imageNode = state.schema.nodes.imageResize.create(found2.node.attrs);
            dispatch2(state.tr.replaceWith(found2.pos, found2.pos + found2.node.nodeSize, imageNode));
          }
          return true;
        },
        /**
  ...
  // resources/js/src/pv-rich-text.entry.js
  var HIGHLIGHT_COLOR = "#fef9c3";
  var VelmImageResize = ImageResize.extend({
    addAttributes() {
      const attrs = this.parent?.() ?? {};
      if (attrs.containerStyle) {
        attrs.containerStyle = {
          ...attrs.containerStyle,
          renderHTML: (attributes) => attributes.containerStyle ? { containerstyle: attributes.containerStyle } : {}
        };
      }
      if (attrs.wrapperStyle) {
        attrs.wrapperStyle = {
          ...attrs.wrapperStyle,
          renderHTML: (attributes) => attributes.wrapperStyle ? { wrapperstyle: attributes.wrapperStyle } : {}
        };
      }
      attrs.width = {
        default: null,
  ...
  function hydrateRichTextDisplayImages(root = document) {
    root.querySelectorAll(".pv-rich-text-display--simple img[containerstyle]").forEach((img) => {
      ...
      if (wrapperStyle && !img.parentElement?.dataset?.pvImageResizeWrap) {
        ...
        wrap2.dataset.pvImageResizeWrap = "true";
        wrap2.setAttribute("style", wrapperStyle);
        img.parentNode?.insertBefore(wrap2, img);
    ...
    return {
      ...
      init() {
        ...
        editor = new Editor({
          ...
          extensions: [
            ...
            index_default.configure({ multicolor: false }),
            index_default2,
            index_default3,
            TaskList,
      ...
      isActive(cmd) {
        ...
        const map2 = {
          ...
          list: () => editor.isActive("bulletList") || editor.isActive("orderedList") || editor.isActive("taskList"),
  ...
  function flushRichTextEditors() {
    document.querySelectorAll("[data-pv-rich-text]").forEach((root) => {
      const data = typeof Alpine !== "undefined" && typeof Alpine.$data === "function" ? Alpine.$data(root) : null;
